...
14. 热修复、插件化
15. Handler、MessageQueue等一套东西。为什么主线程loop不会ANR？
...
Message知道要发到哪个Handler是因为Message把Handler保存到了target。Message内部使用链表进行回收复用。

16. View事件以及View体系相关知识。
17. Android中使用多线程的方法
...
```
13. 两个链表求和，返回结果也用链表表示 1 -> 2 -> 3 + 2 -> 3 -> 4 = 3 -> 5 -> 7
...
```
15. 找出数组中和为给定值的两个元素，如：[1, 2, 3, 4, 5]中找出和为6的两个元素。
...
```
18. 常见排序，如堆排序，快速，归并，冒泡等，还得记住他们的时间复杂度。
...
3. Http和Https
CA证书，中间机构，公钥加密对称秘钥传回服务端，一个明文一个加密，SSL层，中间人攻击，[参考link](https://www.cnblogs.com/wqhwe/p/5407468.html)
4. 视频聊天使用什么协议？
不要答TCP，答RTMP实时传输协议，RTMP在Github也有很多开源实现，建议面这方面的同学可以去了解一下。
5. 你在项目中遇到的一些问题，如何解决，思路是什么？
